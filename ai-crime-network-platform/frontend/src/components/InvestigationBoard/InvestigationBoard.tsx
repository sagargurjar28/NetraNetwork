...
import { nodeTypes, resolveNodeType, type PinData } from './nodes'
...
const DEFAULT_BOARD = '22222222-2222-2222-2222-222222222222'
...
function toRFNode(pin: BoardPin, onRename: PinData['onRename']): RFNode {
  // READ SITE: content.original_type wins over entity_type (see nodes/index.ts).
  return { id: pin.id, type: resolveNodeType(pin), position: { x: pin.x, y: pin.y }, data: { pin, onRename } }
}
...
function BoardCanvas() {
  ...
  const load = useCallback(async (boardId?: string) => {
    const target = boardId || boardRef.current
    setLoading(true)
    try {
      const b = await boardsApi.getBoard(target)
  ...
  function selectCase(caseId: string) {
    // Mock fallback loads the fixture board for a case.
    if (import.meta.env.VITE_USE_MOCKS === 'true') {
      toast('Loaded board for case', 'success')
      void load('22222222-2222-2222-2222-222222222222')
      return
    }
    void (async () => {
      try {
        const b = await boardsApi.getBoardByCase(caseId)
        setNodes(b.pins.map((p) => toRFNode(p, onRename)))
        setEdges(b.connections.map(toRFEdge))
        setSelectedPin(null)
        setSelectedConn(null)
  ...
  function onImportFile(f: File | undefined) {
    ...
    reader.onload = () => {
      try {
        ...
        const okTypes = ['person', 'organization', 'building', 'phone', 'location', 'document', 'note', 'case', 'fir', 'bankaccount']
    ...
    <div className="flex h-[calc(100vh-96px)] gap-3">
      ...
      <Card className="relative flex-1 overflow-hidden bg-base">
        ...
        <BoardToolbar
          ...
          // Local-only. No POST /api/boards/ until case selection is wired.
