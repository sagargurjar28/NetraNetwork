...
import { adaptCitation } from '@/services/copilotService'
...
export function SourceDrawer({ citation, onClose }: { citation: Citation | null; onClose: () => void }) {
  ...
  const c = adaptCitation(citation)
  return (
    <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label="Source">
      ...
      <div className="w-[360px] max-w-[90vw] bg-surface-1 border-l border-[rgba(255,255,255,0.08)] p-5 space-y-3 overflow-auto">
        ...
        <div className="text-xs font-mono text-text-muted">cite:{c.id} • {c.type} • {c.refId}</div>
        <p className="text-sm text-text-secondary leading-relaxed">{c.excerpt}</p>
        <div className="text-xs text-text-muted">Doc ID: <span className="font-mono text-text-primary">{c.type === 'document' ? c.refId : '—'}</span> • Entity ID: <span className="font-mono text-text-primary">{c.type === 'entity' ? c.refId : '—'}</span></div>
        <Link to={sourceHref(c)} onClick={onClose} className="inline-block px-3 py-1.5 rounded-lg bg-accent-primary text-white text-sm hover:bg-accent-hover">Open in domain →</Link>
      </div>
    </div>
  )
}
