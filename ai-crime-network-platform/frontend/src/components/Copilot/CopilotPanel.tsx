...
import { mockCopilotStream, sendCopilotAbort, sendCopilotRegenerate, copilotApi, adaptCitation } from '@/services/copilotService'
    ...
    if (import.meta.env.VITE_USE_MOCKS !== 'true') {
      ...
      copilotApi.sendMessage({
        ...
        (res: { answer?: string; citations?: unknown[] }) => {
          ...
          const citations = (res.citations || []).map((c) => adaptCitation(c as { type: string; id: string; label: string }))
          useCopilotStore.setState({
            conversations: useCopilotStore.getState().conversations.map((c) =>
              c.id === convId ? { ...c, messages: c.messages.map((m) => (m.id === mid ? { ...m, content: text, citations, streaming: false } : m)) } : c,
            ),
            streamingId: null,
            isStreaming: false,
          })
          useCopilotStore.getState().autoTitleFromFirstUserMessage(convId)
          toast('Response complete', 'success')
        },
        (e: { message?: string }) => {
          useCopilotStore.getState().finishStream(convId, mid)
          toast(e?.message || 'Copilot request failed', 'error')
        },
      )
      return mid
    }
